...
public class TableDbTable {
    ...
    public void updateTableData(int id,String name,String days,int isMain,String schedule_start,String schedule_end,int isReplace){
        try {
            ...
            Log.v("更新資料列", String.format("在%s更新一筆資料：%s,%s,%s,%s,%s,%s", SQLiteTable_Name,"課表名稱",name,"課表天數",days,"主要",isMain,"課表開始日",schedule_start,"課表結束日",schedule_end,"時間重複時是否取代",isReplace));
        } catch (Exception e) {
            Log.e("#004", "資料列更新失敗");
        }
    }
