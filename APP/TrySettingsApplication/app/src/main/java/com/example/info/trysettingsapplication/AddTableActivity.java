...
public class AddTableActivity extends AppCompatActivity {
    ...
    public void UpdateAdapter() {
        try {
            SimpleAdapter adapter=new SimpleAdapter(AddTableActivity.this,Time,android.R.layout.simple_list_item_2,new String[]{"start_time","end_time"},new int[]{android.R.id.text1, android.R.id.text2});            ClassTime.setAdapter(adapter);
    ...
    private Button.OnClickListener Button_Listener= new Button.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            TimePickerDialog timePicker=new TimePickerDialog(AddTableActivity.this, new TimePickerDialog.OnTimeSetListener() {
                @Override
            ...
            timePicker= new TimePickerDialog(AddTableActivity.this, new TimePickerDialog.OnTimeSetListener() {
                @Override
                public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
    ...
    private boolean check(){
        ...
        if(TableDb.getCursor(String.format("課表名稱='%s' AND _id !=%d ",Name.getText().toString(),Table_id)).getCount()>0){
            Toast.makeText(AddTableActivity.this,"課表名稱已存在",Toast.LENGTH_LONG).show();
            Log.w("課表名稱錯誤","課表名稱已存在");
            return false;
        } //補習班 學校
        if(!IsStringtoCalendar(Start_date.getText().toString())){
            Toast.makeText(AddTableActivity.this,"課表開始日期格式錯誤",Toast.LENGTH_LONG).show();
            Log.w("課表開始日期錯誤","課表開始日期格式錯誤");
            return false;
        }
        if(!IsStringtoCalendar(End_date.getText().toString())){
            Toast.makeText(AddTableActivity.this,"課表結束日期格式錯誤",Toast.LENGTH_LONG).show();
        ...
        if (End_date.getText().toString().equals("")||!compareDate(Start_date.getText().toString(),End_date.getText().toString())){
            Toast.makeText(AddTableActivity.this,"課表日期錯誤",Toast.LENGTH_LONG).show();
            Log.w("課表日期錯誤","課表日期錯誤");
            return false;
        }
        ...
        try{
            ...
            Toast.makeText(AddTableActivity.this,"　",Toast.LENGTH_LONG).show();
            return false;
        }
        return true;
    }
    ...
    private EditText.OnClickListener DatePick_Listener= new EditText.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (v==Start_date){
                dataPick=new DatePickerDialog(AddTableActivity.this,datepicker,
                ...
                dataPick=new DatePickerDialog(AddTableActivity.this,datepicker2,
                        m_Calendar.get(Calendar.YEAR),
                        m_Calendar.get(Calendar.MONTH),
                        m_Calendar.get(Calendar.DAY_OF_MONTH));
                dataPick.show();
    ...
    private Calendar StringtoCalendar(String date){
        ...
        try{
            ...
            Toast.makeText(AddTableActivity.this,"日期格式不符合 yyyy-MM-dd",Toast.LENGTH_SHORT).show();
        }
        return Calendar;
    }
    ...
    private Calendar StringtoTCalendar(String date){
        ...
        try{
            ...
            Toast.makeText(AddTableActivity.this,"時間格式不符合 hh:mm",Toast.LENGTH_SHORT).show();
        }
        return Calendar;
    }
