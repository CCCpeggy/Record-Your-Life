...
public class NoteScheduleDbTable {private String SQLiteDB_Path = null;
    ...
    public NoteScheduleDbTable(String path, SQLiteDatabase Database) {
        SQLiteDB_Path = path;
        db = Database;
    }
    ...
    public void updateNoteScheduleData(int id,int note){
        try {
            ContentValues row = new ContentValues();
            row.put("提醒ID", note);
    ...
    public void deleteNoteScheduleData(int Delete_id) { //不用改
        try {
    ...
    public void AddNoteScheduleData(){
        int note[]={18,11,6,10,7,5,16,7,10,8,2,19,17,16,8,9,3,7,1};
        for(int i=0;i< note.length;i++){
            insertNoteScheduleData(note[i]);
        }
    }
